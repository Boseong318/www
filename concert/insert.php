...
<?
	...
	//수정글쓰기  $table , 폼입력값 , 세션변수, $num, $mode == modify, $page
	...
	// 다중 파일 업로드 upfile : 첨부된 파일들 -> 배열 형태로 돼 있음
	$files = $_FILES["upfile"];		//길이 값을 $files에 담아놓음
     //$files[0] , $files[1], $files[2]
	$count = count($files["name"]); //업로드된 파일개수 3, count : 배열의 개수 계산
	...
	for ($i=0; $i<$count; $i++)
	{
		...
		$upfile_tmp_name[$i] = $files["tmp_name"][$i];	//임시파일 .tmp 
		$upfile_type[$i]     = $files["type"][$i]; 		//파일 타입
	...
	if ($mode=="modify")		//수정 글 쓰기(좀 복잡함)
	{
		$num_checked = count($_POST['del_file']);		// count : ($_POST['del_file'])의 개수(3)
		...
		for ($i=0; $i<$count; $i++)					// update DB with the value of file input box
		{
			...
			$org_name_value = $upfile_name[$i];		//$org_name_value = a1.jpg
			$org_real_value = $copied_file_name[$i];//$org_real_value = 2019_03_22_10_07_15_0.jpg
			if ($del_ok[$i] == "y")	//삭제 체크 했다면
			{
				$delete_field = "file_copied_".$i;	//file_copied_0
				$delete_name = $row[$delete_field];	// $row[file_copied_0];	->2022_02_22_10_43_20_0.jpg
				
				$delete_path = "./data/".$delete_name;	//'./data/2022_02_22_10_43_20_0.jpg'
			...
			else		//첨부파일 삭제 안 하고 다른 파일로 바꿨을 때 새로운 파일로 업데이트 하는 코드
			{
				if (!$upfile_error[$i])
				{
					$sql = "update $table set $field_org_name = '$org_name_value', $field_real_name = '$org_real_value'  where num=$num";
