<? 
	...
	for ($i=0; $i<3; $i++)
	{
		if ($image_copied[$i]) //업로드한 파일이 존재하면 0 1 2, $image_copied : 저장된 파일 이름
		{
			$imageinfo = GetImageSize("./data/".$image_copied[$i]);
            //GetImageSize(서버에 업로드된 파일 경로 파일명): 파일의 너비와 높이값, 종류를 찾아냄
			// 왜 필요? : 
			$image_width[$i] = $imageinfo[0];  //파일너비
			$image_height[$i] = $imageinfo[1]; //파일높이
			$image_type[$i]  = $imageinfo[2];  //파일종류

        if ($image_width[$i] > 785)		//785px는 게시판 레이아웃의 너비
				$image_width[$i] = 785;
		}
		else		
		{
...
<? include "../common/sub_header.html" ?>
	...
	<div class="sub_menu">
		<ul>
			<li class="current"><a href="../concert/list.php">뉴스</a></li>
	...
	<article id="content">
		<div class="title_area">
			<div class="line_map">home &gt; 기업홍보 &gt; <strong>뉴스</strong></div>
			<h2>뉴스</h2>
			<p>news</p>
		</div>
		...
		<div id="view_button">
			...
			<? 
				if($userid==$item_id || $userid=="bsbo318" || $userlevel==1 )
				{
			?>
