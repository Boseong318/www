...
<?
    ...
    @extract($_GET);
    @extract($_SESSION);
    //수정=>mode=modify&num=7&page=1	/	$subject = 제목 (post)	/	$content = 내용글 (post)
  

	if(!$userid) {		//로그인이 안 됐을 떄
		echo("
		<script>
	...
	else  //새글쓰기
	{
		...
		$subject = str_replace("'", "&#039;", $subject);
		$content = str_replace("'", "&#039;", $content);
	 // htmlspecialchars: "(&quot;) '(&#039;) &(&amp;) <(&lt;) >(&gt;) -> 태그로 알아서 처리 해주는 태그
	 // str_replace : ' 는 &#039 로 바꿔서 넣으세요~ 특수 문자 각각 써줘야 함
