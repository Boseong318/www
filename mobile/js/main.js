...
$(document).ready(function() {
    ...
    var cnt=1;  //이미지 순서 1 2 3 4 5 4 3 2 1 2 3 4 5 ...(주인공, 현재 이미지 순서)
    var direct=1;  //1씩 증가(+1)/감소(-1) 방향타
    var position=0; //겔러리 무비의 left값 0 -1000 -2000 -3000 -4000
    var onoff=true; // true=>타이머 동작중 , false=>동작하지 않을때
      
    $('.btn1').css('background','#01a0ca'); //첫번째 불켜
    $('.btn1').css('width','30');
   ...
   function moveg(){
        cnt+=direct;  //카운트가 1 2 3 4 5 4 3 2 1 2 3 4 5 ......
         ...
         //     position=-1000;
          ...
          //    position=-4000;
          // }
        position = -(cnt-1)*800;  //  cnt에 해당하는 left값(목적지) 처리
        ...
        $('.btn'+cnt).css('background','#01a0ca');//자신만 불켜
    ...
    timeonoff= setInterval( moveg , 3000); //4초마다 자동기능 
    ...
    $('.mbutton').click(function(event){  //각각의 버튼을 클릭한다면...
     ...
     $('.mbutton').css('width','16'); 
      ...
      $('.btn'+cnt).css('background','#01a0ca');//자신 버튼만 불켜 
      $('.btn'+cnt).css('width','30');
    ...
    $('.ps').click(function(){ 
         if(onoff==true){
            ...
            timeonoff= setInterval( moveg , 3000); //play버튼 클릭시
