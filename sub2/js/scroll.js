$(document).ready(function () {
    ...
    $('.tap_menu li:eq(0)').find('a').addClass('spy');
    //첫번째 서브메뉴 활성화
    ...
    var h2= $('.content_area div:eq(2)').offset().top-500 ;
    ...
    $(window).on('scroll',function(){
        ...
        if(scroll>smh){         //메인 비주얼의 높이보다 스크롤 값이 크면 스티키 메뉴가 나타난다!
            $('.content').addClass('navOn');     //코드 순서 상, 아래에 있는 navOn이 붙으면서 가서 붙어라!
            //스크롤의 거리가 350px 이상이면 서브메뉴 고정
            $('header').hide();                 //원래 있던 헤더 보이지 마라!
        }else{
            $('.content').removeClass('navOn');      //navOn 버리고 원래 상태로 돌아간다.
